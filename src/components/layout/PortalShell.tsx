...
export function PortalShell({role,userName,children}:{role:PortalRole;userName:string;children:ReactNode}){
 const pathname=usePathname(); const router = useRouter(); const nav=siteConfig.portalNav[role];
 ...
 return <div className="min-h-screen bg-[#f5f7f5] md:flex">
   <aside className="hidden w-[238px] shrink-0 flex-col bg-brand-green text-white md:flex">
    ...
    <div className="mt-auto border-t border-white/10 px-5 py-4"><div className="truncate text-xs font-semibold">{userName}</div><div className="mt-1 text-[9px] text-white/45">{roleDescriptions[role]}</div><button onClick={handleLogout} className="mt-3 inline-block text-[9px] font-semibold text-white/65 hover:text-white">Log out →</button></div>
   </aside>
   <div className="min-w-0 flex-1">
    <header className="sticky top-0 z-30 flex h-[64px] items-center justify-between border-b border-[#e1e7e4] bg-white/95 px-4 backdrop-blur sm:px-7"><div className="md:hidden"><span className="font-display text-base font-semibold text-brand-green">ilearn<span className="text-brand-orange">4u</span></span></div><div className="hidden text-[10px] text-ink-soft md:block">{roleDescriptions[role]}</div><div className="flex items-center gap-3"><span className="truncate text-[10px] font-medium text-ink-soft sm:block">{userName}</span><button onClick={handleLogout} className="rounded-md border border-[#dbe3df] px-3 py-2 text-[10px] font-semibold text-brand-green hover:bg-[#f7f9f8]">Log out</button></div></header>
